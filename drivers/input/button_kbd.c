...
/* The key acts as a modifier while it is held down */
#define BUTTON_CHORD_MODIFIER	KEY_ENTER
...
static const struct {
	int code;	/* keycode reported by button_get_code() */
...
static int button_chord_code(int code)
{
	int i;
	/* Return the chorded keycode for @code, or 0 */
	for (i = 0; i < ARRAY_SIZE(chord_map); i++)
		if (chord_map[i].code == code)
...
int button_read_keys(struct input_config *input)
{
	...
	uclass_id_foreach_dev(UCLASS_BUTTON, button_gpio_devp, uc) {
		...
		if (CONFIG_IS_ENABLED(BUTTON_REMAP_PHONE_KEYS)) {
			...
			chorded_code = button_chord_code(code);
			if (chorded_code) {
